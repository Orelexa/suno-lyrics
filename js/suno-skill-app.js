...
class SunoSkillApp {
    ...
    async generateStructuredLyrics() {
        ...
        try {
            ...
            // Ha a referencia magyar, akkor az Output 2 is legyen angol
            const isHuRef = this.isHungarianText(referenceLyrics);
            const englishOut = result.english || '';
            const hungarianOut = isHuRef ? englishOut : (result.hungarian || '');

            // Eredmény megjelenítése
            this.generatedLyrics.english = englishOut;
            this.generatedLyrics.hungarian = hungarianOut;

            document.getElementById('englishLyrics').value = englishOut;
            document.getElementById('hungarianLyrics').value = hungarianOut;
...
===========================================`;
    ...
    // Egyszerű magyar nyelv detektálás a referencia szöveghez
    isHungarianText(text) {
        if (!text) return false;
        const huChars = /[áéíóöőúüűÁÉÍÓÖŐÚÜŰ]/;
        const huWords = /(\b|\s)(és|hogy|nem|vagy|mert|egy|az|van|vagyok|lesz|volna|mint|akkor|kell)(\b)/i;
        const hasHuChars = huChars.test(text);
        const hasHuWords = huWords.test(text);
        return hasHuChars || hasHuWords;
    }
}
